...
local RENDER_CONSTANTS = {
    movementIndicator = {
        minDistance = 30, -- Minimum distance to show movement indicator
        markerSize = 10,  -- Size of target crosshair
    },
    velocity = {
        threshold = 5, -- Min speed to display velocity vector
        scale = 0.5,   -- Scale factor for velocity visualization
    },
    targetRing = {
        padding = 8,    -- Extra padding around entity for target ring
        colorMix = 0.7, -- Color mixing ratio for locked state
    },
    hoverOutline = {
        padding = 6, -- Extra padding around entity for hover outline
    },
}
...
local function drawMovementIndicator(player, colors)
    ...
    local markerSize = RENDER_CONSTANTS.movementIndicator.markerSize
...
local function drawTargetIndicator(colors, combatSystem, camera)
    ...
    if shader and (isLocking or isLocked) then
        ...
        local mix = RENDER_CONSTANTS.targetRing.colorMix
        sr = sr * (1 - mix) + 1.0 * mix
        sg = sg * (1 - mix) + 0.6 * mix
        sb = sb * (1 - mix) + 0.2 * mix
...
function game_render.load()
    ...
    if ok then
        ...
        print("[WARNING] Failed to load lock_on shader:", shader)
    end
end
